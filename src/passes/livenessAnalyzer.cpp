...
LivenessAnalyzer::LivenessAnalyzer(const bool verbose_flag) : FunctionPass(ID), verbose(verbose_flag) {}
LivenessAnalyzer::~LivenessAnalyzer() { releaseMemory(); }
...
void LivenessAnalyzer::print(raw_ostream& o, const Module *M) const {
...
void LivenessAnalyzer::getBlockInternalLiveInValues(Instruction* inst, LiveSetType& liveVals) {
...
void LivenessAnalyzer::removeBlockInternalNonLiveInValues(Instruction* frontier, LiveSetType& liveInVals, const LiveSetType& liveOutVals) {
	assert (frontier);
...
// this is so ugly... =)
void LivenessAnalyzer::mapLiveValues(Function* f, Function* newF, ValueMap<const Value*, Value*>& valueMap) {
	assert (f && newF);
...
void LivenessAnalyzer::computeLiveValues(Function* f) {
	assert (f);
...
void LivenessAnalyzer::computeLivenessInformation(BasicBlock* block, Value* def, Instruction* use, std::set<BasicBlock*>& visitedBlocks) {
