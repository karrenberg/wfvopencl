...
#include "TestDynCheckSpeed.hpp"

using namespace dct;

int 
TestDynCheckSpeed::setupTestDynCheckSpeed()
{
...
int 
TestDynCheckSpeed::genBinaryImage()
{
    ...
    kernelPath.append("TestDynCheckSpeed_Kernels.cl");
...
int
TestDynCheckSpeed::setupCL(void)
{
    cl_int status = 0;
    ...
    kernel = clCreateKernel(program, "TestDynCheckSpeed", &status);
    if(!sampleCommon->checkVal(
            status,
            CL_SUCCESS,
...
int 
TestDynCheckSpeed::runCLKernels(void)
{
    cl_int   status;
    cl_event events[2];
    ...
    /** 
     * 1st argument to the kernel which stores the output of the TestDynCheckSpeed
     */
    ...
    status = clSetKernelArg(
                    kernel, 
                    3, 
                    blockWidth * blockWidth * sizeof(cl_int),
                    NULL);
    if(!sampleCommon->checkVal(
            status,
            CL_SUCCESS,
    ...
    /**
     * 7th argument to the kernel , flag to perform inverse TestDynCheckSpeed
     */
    status = clSetKernelArg(
                    kernel, 
                    6, 
...
cl_uint
TestDynCheckSpeed::getIdx(cl_uint blockIdx, cl_uint blockIdy, cl_uint localIdx, cl_uint localIdy, cl_uint blockWidth, cl_uint globalWidth)
{
 ...
 */
void 
TestDynCheckSpeed::TestDynCheckSpeedCPUReference( cl_int * verificationOutput,
                      const cl_int * input ,
...
{
    cl_int * temp = (cl_int *)malloc(width*height*sizeof(cl_int));
...
int TestDynCheckSpeed::initialize()
{
...
int TestDynCheckSpeed::setup()
{
    ...
    if(setupTestDynCheckSpeed()!=SDK_SUCCESS)
        return SDK_FAILURE;
...
int TestDynCheckSpeed::verifyResults()
{
    if(verify)
    {
    }
...
void TestDynCheckSpeed::printStats()
{
    ...
    this->SDKSample::logStats(totalKernelTime, "TestDynCheckSpeed", vendorName);
}

int TestDynCheckSpeed::cleanup()
{
...
{
    TestDynCheckSpeed clTestDynCheckSpeed("OpenCL TestDynCheckSpeed");

    clTestDynCheckSpeed.initialize();
    if(!clTestDynCheckSpeed.parseCommandLine(argc, argv))
        return SDK_FAILURE;

    if(clTestDynCheckSpeed.isDumpBinaryEnabled())
    {
        return clTestDynCheckSpeed.genBinaryImage();
    }
    else
    {
        if(clTestDynCheckSpeed.setup()==SDK_FAILURE)
            return SDK_FAILURE;
        int state = clTestDynCheckSpeed.run();
        if(state == SDK_FAILURE)
            return SDK_FAILURE;
        else if(state == SDK_SUCCESS)
        {
            if(clTestDynCheckSpeed.verifyResults()==SDK_FAILURE)
                return SDK_FAILURE;
        }
        if(clTestDynCheckSpeed.cleanup()==SDK_FAILURE)
            return SDK_FAILURE;
        clTestDynCheckSpeed.printStats();
    }
