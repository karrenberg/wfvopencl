...
BlackScholes::setupCL(void)
{
    ...
    const char* source = "BlackScholes_Kernels.bc";
...
BlackScholes::verifyResults()
{
    if(verify)
    {
        ...
        if(!sampleCommon->compare(hostCallPrice, deviceCallPrice, width * height * 4))
        {
            std::cout << "Failed\n";
            return SDK_FAILURE;
        }
        ...
        if(!sampleCommon->compare(hostPutPrice, devicePutPrice, width * height * 4))
        {
            std::cout << "Failed\n";
            return SDK_FAILURE;
        }
