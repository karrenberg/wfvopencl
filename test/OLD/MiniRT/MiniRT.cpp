...
int MiniRT::setupCL()
{
    ...
    */
    ...
    //kernelPath.append("MiniRT_Kernels.cl");
    ...
    const char * source = "MiniRT_Kernels.bc";
...
#else
	glRasterPos2d(-1,-1);
	glDrawPixels(clMiniRT->getWidth(), clMiniRT->getHeight(), GL_RGBA, GL_UNSIGNED_BYTE, teximage);
#endif
	glutSwapBuffers();
...
{
	clMiniRT = new MiniRT("OpenCL MiniRT");
	teximage = new unsigned char[clMiniRT->getWidth() * clMiniRT->getHeight() * 4];

	//MiniRT clMiniRT("OpenCL MiniRT");
	//me = &clMiniRT;

	if(clMiniRT->initialize() != SDK_SUCCESS)
        return SDK_FAILURE;
    if(!clMiniRT->parseCommandLine(argc, argv))
        return SDK_FAILURE;
    if(clMiniRT->setup() != SDK_SUCCESS)
        return SDK_FAILURE;
    //if(clMiniRT.run() != SDK_SUCCESS)
        //return SDK_FAILURE;
	
	glutInit(&argc, argv);
	glutInitWindowSize(clMiniRT->getWidth(), clMiniRT->getHeight());
	glutInitDisplayMode(GLUT_RGBA | GLUT_DEPTH | GLUT_DOUBLE);
	glutCreateWindow("OpenCL AO");
	initGL();
    ...
    if(clMiniRT->cleanup()!=SDK_SUCCESS)
        return SDK_FAILURE;
