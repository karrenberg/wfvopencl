...
#include "MandelbrotSimpleDisplay.hpp"
...
cleanup(void)
{
    if(pixels)
        free(pixels);
    clMandelbrotSimple.cleanup();
    clMandelbrotSimple.printStats();
...
main(int argc, char * argv[])
{
    ...
    clMandelbrotSimple.initialize();
    if(!clMandelbrotSimple.parseCommandLine(argc, argv))
        return SDK_FAILURE;
    if(clMandelbrotSimple.setup()!=SDK_SUCCESS)
        return SDK_FAILURE;
    if(clMandelbrotSimple.run()!=SDK_SUCCESS)
        return SDK_FAILURE;
    if(clMandelbrotSimple.verifyResults()!=SDK_SUCCESS)
        return SDK_FAILURE;

    /* show window if it is not running in quiet mode */
    if(clMandelbrotSimple.showWindow())
    {
