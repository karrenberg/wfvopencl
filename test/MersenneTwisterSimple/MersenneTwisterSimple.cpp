...
#include "MersenneTwisterSimple.hpp"
...
int 
MersenneTwisterSimple::setupMersenneTwisterSimple()
{
...
#if defined (_WIN32)
    seeds = (cl_uint*)_aligned_malloc(width * height * sizeof(cl_uint), 16);
#elif defined (__APPLE__)
    seeds = (cl_uint*)malloc(width * height * sizeof(cl_uint));
#else
    seeds = (cl_uint*)memalign(16, width * height * sizeof(cl_uint));
#endif
    ...
    deviceResult = (cl_float *) malloc(width * height * mulFactor * sizeof(cl_float));
    ...
    memset((void*)deviceResult, 0, width * height * mulFactor * sizeof(cl_float));

	return SDK_SUCCESS;
}

int 
MersenneTwisterSimple::genBinaryImage()
{
    ...
    kernelPath.append("MersenneTwisterSimple_Kernels.cl");
...
int
MersenneTwisterSimple::setupCL(void)
{
    cl_int status = 0;
    ...
    resultBuf = clCreateBuffer(context,
                               CL_MEM_WRITE_ONLY,
                               width * height * sizeof(cl_float) * mulFactor,
                               NULL,
                               &status);
	...
	const char * source = platformIsPacketizedOpenCL ?
		"MersenneTwisterSimple_Kernels.bc" :
...
int
MersenneTwisterSimple::runCLKernels(void)
{
    cl_int   status;
    cl_event events[1];
    ...
    status = clEnqueueReadBuffer(commandQueue,
                                 ...
                                 CL_TRUE,
                                 0,
                                 width * height * mulFactor * sizeof(cl_float),
                                 deviceResult,
                                 0,
                                 NULL,
                                 &events[0]);
...
int MersenneTwisterSimple::initialize()
{
...
int MersenneTwisterSimple::validateCommandLineOptions()
{
...
int MersenneTwisterSimple::setup()
{

    if(setupMersenneTwisterSimple() != SDK_SUCCESS)
        return SDK_FAILURE;
...
int 
MersenneTwisterSimple::verifyResults()
{
    if(verify)
    {
        ...
        for(int i = 0; i < height * width * (int)mulFactor; ++i)
        {
            meanVal += deviceResult[i];
        }
        
        meanVal = fabs(meanVal) / (height * width * (int)mulFactor);
...
void 
MersenneTwisterSimple::printStats()
{
    ...
    stats[0] = sampleCommon->toString(height * width * mulFactor, std::dec);
    stats[1] = sampleCommon->toString(totalTime, std::dec);
    stats[2] = sampleCommon->toString(kernelTime, std::dec);
    stats[3] = sampleCommon->toString(height * width * mulFactor / totalTime, std::dec);
    
    this->SDKSample::printStats(strArray, stats, 4);
    this->SDKSample::logStats(strArray, stats, 4, "MersenneTwisterSimple", vendorName);
}

int
MersenneTwisterSimple::cleanup()
{
...
main(int argc, char * argv[])
{
    ...
    MersenneTwisterSimple clMersenneTwisterSimple("Mersenne Twister sample");

    /* Initialization */
	if(clMersenneTwisterSimple.initialize()!=SDK_SUCCESS)
		return SDK_FAILURE;

    /* Parse command line options */
	if(!clMersenneTwisterSimple.parseCommandLine(argc, argv))
		return SDK_FAILURE;

    /* Validate the command line options */
    if(!clMersenneTwisterSimple.validateCommandLineOptions())
        return SDK_FAILURE;

    if(clMersenneTwisterSimple.isDumpBinaryEnabled())
    {
        return clMersenneTwisterSimple.genBinaryImage();
    }
    else
    {
	    /* Setup */
        if(clMersenneTwisterSimple.setup()!=SDK_SUCCESS)
		    return SDK_FAILURE;

        /* Run */
	    if(clMersenneTwisterSimple.run()!=SDK_SUCCESS)
		    return SDK_FAILURE;

        /* Verifty */
        if(clMersenneTwisterSimple.verifyResults()!=SDK_SUCCESS)
		    return SDK_FAILURE;

        /* Cleanup resources created */
	    if(clMersenneTwisterSimple.cleanup()!=SDK_SUCCESS)
		    return SDK_FAILURE;

        /* Print performance statistics */
	    clMersenneTwisterSimple.printStats();
    }
