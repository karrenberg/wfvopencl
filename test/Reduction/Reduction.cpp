...
#if defined (_WIN32)
    input = (cl_uint*)_aligned_malloc(length * sizeof(cl_uint), 16);
#else
    input = (cl_uint*)memalign(16, length * sizeof(cl_uint));
#endif
...
Reduction::setupCL()
{
    ...
    const char * source = "Reduction_Kernels.bc";
...
Reduction::runCLKernels()
{
...
#if defined (_WIN32)
    tempOut = (cl_uint*)_aligned_malloc(numBlocks * sizeof(cl_uint), 16);
#else
    tempOut = (cl_uint*)memalign(16, numBlocks * sizeof(cl_uint));
#endif
    ...
    for(int i = 0; i < iterations; i++)
    {
        ...
        /* local array */
        status = clSetKernelArg(kernel, 2, groupSize * sizeof(cl_uint), NULL);
        if(!sampleCommon->checkVal(status, 
                                   CL_SUCCESS,
